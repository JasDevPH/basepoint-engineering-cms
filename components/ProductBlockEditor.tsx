...
export default function ProductBlockEditor({
  ...
  const blockTypeConfig = {
    ...
    "quote-form": { icon: FileQuestion, color: "yellow", label: "Quote Form" },
          ...
          {Object.entries(blockTypeConfig).map(([type, config]) => {
            ...
            return (
              <button
                key={type}
                type="button"
                onClick={() => addBlock(type as ProductContentBlock["type"])}
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50 rounded-t-xl">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-gray-900 capitalize">
                    {config?.label || block.type.replace("-", " ")}
                  </span>
                </div>
                <button
                  type="button"
                        ...
                        <div key={specIndex} className="flex gap-2">
                          ...
                          {block.specs && block.specs.length > 1 && (
                            <button
                              type="button"
                              onClick={() =>
                                deleteSpecItem(block.id, specIndex)
                              }
                              className="p-2.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors"
                            >
                              <X className="w-4 h-4" />
                ...
                {block.type === "quote-form" && (
                  <div className="p-6 bg-gradient-to-br from-yellow-50 to-amber-50 rounded-lg border-2 border-yellow-200">
                    ...
                    <p className="text-sm text-gray-600 mb-4">
                      A quote form will be displayed here (styled based on your
                      design)
                    </p>
                    <input
                      type="text"
                      ...
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent outline-none bg-white"
                    />
                  </div>
                )}
